...
def downloadtv():
    with open('link.bat', 'w') as down_load:
        down_load.write(f'youtube-dl -f "bestvideo[height=1080]+bestaudio/best[height=1080]" {link.get()}')
        down_load.close()
    runit()
f = Frame(root)
f.grid()
...
Button(f1, text='Download', padx=50, relief=RAISED, font=10, borderwidth=5, command=downloadtv).grid(column=1, row=5)
